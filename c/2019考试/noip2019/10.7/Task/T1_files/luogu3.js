...
function login() {
    var username, password, cookie, redirect, two_factor;
    username = $("[name=username]").val();
    password = $("[name=password]").val();
    redirect = $("[name=redirect]").val();
    cookie = $("[name=cookie]:checked").val();
    two_factor = $("#login-2fa-prompt").find("input[type=text]").val();
        ...
        $.post("/login/loginpage", {
            ...
            function (data) {
                ...
                if (res["code"] == 401) {
                    ...
                    $("#login-2fa-prompt").modal({
                        onConfirm: login
                    })
                } else if (res["code"] == 403.2) {
                    setTimeout(function () {
                        $("[name=login]").removeClass("am-disabled");
                        $("#login-2fa-prompt").find(".am-modal-hd:first").html(res['message']);
                        $("#login-2fa-prompt").find("input[type=text]").val("");
                        $("#login-2fa-prompt").modal({
                            onConfirm: login
                        })
                    }, 500);
