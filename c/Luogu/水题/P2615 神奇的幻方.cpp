...
输出格式：
共 NN 行 ，每行 NN 个整数，即按上述方法构造出的 $N 、times N$ 的幻方，相邻两个整数之间用单空格隔开。
...
int main()
{
	...
	int mid = n / 2 + 1;
	ans[1][mid] = 1;
	mapp[1] = make_pair(1,mid);
	for(int i = 2;i <= n * n;i++)
	{
		pair <int,int> seat = mapp[i - 1];
		if(seat.first == 1 && seat.second != n)// (K-1)(K−1) 在第一行但不在最后一列，则将 KK 填在最后一行， (K-1)(K−1) 所在列的右一列；
		{
			pair <int,int> s = make_pair(n,seat.second + 1);
			mapp[i] = s;
			ans[n][seat.second + 1] = i;
		}
		else if(seat.first != 1 && seat.second == n)//若 (K-1)(K−1) 在最后一列但不在第一行，则将 KK 填在第一列， (K-1)(K−1) 所在行的上一行；
		{
			pair <int,int> s = make_pair(seat.first - 1,1);
			mapp[i] = s;
			ans[seat.first - 1][1] = i;
		}
		else if(seat.first == 1 && seat.second == n)//若 (K-1)(K−1) 在第一行最后一列，则将 KK 填在 (K-1)(K−1) 的正下方；
		{
			pair <int,int> s = make_pair(seat.first + 1,seat.second);
			mapp[i] = s;
			ans[seat.first + 1][seat.second] = i;
		}
		else//若 (K-1)(K−1) 既不在第一行，也不在最后一列
		{
			if(ans[seat.first - 1][seat.second + 1] == 0)//如果 (K-1)(K−1) 的右上方还未填数，则将 KK 填在 (K-1)(K−1) 的右上方
				ans[seat.first - 1][seat.second + 1] = i,
				mapp[i] = make_pair(seat.first - 1,seat.second + 1);
			else//否则将 LL 填在 (K-1)(K−1) 的正下方
				ans[seat.first + 1][seat.second] = i,
				mapp[i] = make_pair(seat.first + 1,seat.second);
		}
	}
	for(int i = 1;i <= n;i++)
	{
